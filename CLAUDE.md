...
- **Match a field's own insert contract, not its neighbour's.** In `update_finding`, `status` normalizes and accepts aliases (`resolve_finding_status`) while `severity` is exact-match, because that is what `add_finding` has always enforced for severity. Making the update path lenient while insert stayed strict would create a worse, same-field inconsistency. CB-19 tracks unifying them across `add`, `batch_add`, `update` and CSV import *together*; doing it per-site just moves the seam.
