...
- **A column settable at INSERT should be settable at UPDATE, or documented as immutable.** `severity` was write-once on findings while the sibling `priority` was already mutable on requirements (CB-17) — so a card whose impact was re-measured after filing could not be corrected in the structured field, and the correction moved into a free-text note, which is exactly the state a tracker exists to prevent. `reported_at_commit` is the worked example of the other branch: deliberately immutable, and *said so* in the docstring. Decide which one a new column is, and write it down. Note the two entities must be checked against each other, not each against itself — the asymmetry was invisible from inside `findings.py`.
...
- **A CLI handler must never catch `ValueError` broadly — `json.JSONDecodeError` subclasses it.** A domain update commits its write and only *then* raises while serializing the return value from a row with malformed stored `meta`/`tags`. Folding that into the validation arm prints a tidy one-line error and exits 1 for a mutation that **already landed** — a failure-shaped signal for a successful write, the same class of lie as CB-15/CB-16. `_cmd_update` re-raises `json.JSONDecodeError` in an arm placed *before* the `(KeyError, ValueError)` arm; order is load-bearing, and `tests/test_findings.py::TestRetriageCliContract` pins it.
