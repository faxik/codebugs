...
- ~~`blockers.py` cross-module reach into private `_row_to_dict`~~ — **resolved.** `blockers.py` calls the public `db.row_to_dict()` and does not reach into `reqs` at all; no private `_row_to_dict` exists anywhere in the package (CB-5).
...
on the module name: `src/codebugs/db.py` carries `from urllib.request import pathname2url`, a pure
string function that opens nothing, while `import urllib.request` binds the module and hands you
