...
# 2. pre-merge-commit — the half pre-commit structurally cannot cover (CB-57).
#
#    git does not run pre-commit for a merge it completes itself, and the
#    pre-commit hook deliberately exits 0 when MERGE_HEAD exists so a
#    CONFLICTED merge can be finished by hand. So until this hook existed,
#    `git merge <untyped-branch>` onto main was read by nothing: merge.ff=false
#    gave it a merge commit and no mechanism looked at the name.
#
#    Same symlink discipline and the same reason as [1/3].
echo ""
echo "[2/3] pre-merge-commit hook"
...
#
#    It is also what makes [2/3] reachable at all: a fast-forward creates no
