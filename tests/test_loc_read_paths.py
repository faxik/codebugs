...
class TestTheNewLineNumberIsAnswered:
    """The OTHER half of the owner's sentence, and until CB-166 nothing held it.

    What he asked for (CB-95, verbatim): "a more stable and robust fingerprint of
    location than just a line in a file? Ideally, WITH AUTO-RESOLUTION OF THE NEW
    LINE." The class above pins where the CODE went; it cannot pin the number,
    and not by oversight — its fixture is a ``git mv``, which moves a file without
    moving one line inside it, so the resolved line equals the stored line there
    and every assertion about it would pass against a build that recomputed
    nothing. The fixture that discriminates is an edit ABOVE the anchor in the
    SAME file, and that is what this class supplies.

    Measured on this tree, two mutants inside ``loc._summary``:

    * ``"line": None`` while ``"resolved"`` stays ``True`` — the summary says "I
      looked, here is the answer" and carries no answer. The whole suite stayed
      GREEN: 2436 passed, this battery's 39 included.
    * ``resolution["line"] + 1`` — three tests went red and ALL THREE for the
      wrong reason. They run on paths where ``resolution["line"]`` is ``None``,
      so ``+ 1`` raises ``TypeError``, the seam catches it and stamps the row
      ``unavailable``; what they caught was a crashing extension, not an off-by-
      one. On a healthy resolution nothing saw it.
    ...
    @staticmethod
    def _push_down(root, n):
        """Insert `n` lines ABOVE the anchored span, leaving the path alone.

        Deliberately not `git mv`: the whole point is a coordinate that moved
        while the file did not, which is the one shape the path assertions in the
        class above are structurally blind to.
        """
        (root / "f.py").write_text("".join(f"# inserted {i}\n" for i in range(n)) + _body())
        _git(root, "add", "-A")
    ...
    def test_a_line_pushed_down_by_an_edit_above_it_is_read_back_at_its_new_number(
        ...
        """The owner's acceptance for the LINE, through the plain call.

        No flag, no `project_dir`, no second tool: `get` with the id and nothing
        else, which is what a human or an agent actually types.
        """
        root, conn = tracker
        fid = _file(
            conn,
            root,
            meta={"line": 4},
            description="line four of f.py is wrong, filed before anything lands above it",
        )
        self._push_down(root, 3)

        anchor = findings.get_finding(conn, fid)["anchor"]
        # The healthy state is asserted BEFORE the number. Without this the
        # number below could be read off a degraded summary and a failure for
        # the wrong reason would look like a pass for the right one — which is
        # exactly how the `+ 1` mutant was "caught" before this class existed.
        assert anchor["state"] == "anchored"
        assert anchor["resolved"] is True
        assert anchor["loc_status"] == "moved"
        assert anchor["moved_file"] is False
        assert anchor["path"] == "f.py"
        # THE assertion, as a literal. 4 + 3 = 7 is not recomputed here: a test
        # that redoes the arithmetic it is testing agrees with any answer.
        assert anchor["line"] == 7
        assert anchor["end"] == 7
    ...
    def test_each_row_of_a_page_gets_its_own_number_and_not_the_first_one(self, tracker):
        ...
        whole page — so the failure this pins is one only a page can have: an
        answer computed for the first anchored row and handed to the rest. Two
        rows whose true numbers DIFFER is the smallest fixture that can see it;
        one row, or two rows sharing a number, cannot.
        """
        root, conn = tracker
    ...
    def test_a_resolved_coordinate_is_an_integer_whenever_the_status_names_one(
        ...
        """The property, stated over a mixed page — a strictly different mutant
        from the two above, which is why it is not replaced by them.

        The condition is `loc_status`, NOT `resolved`, and that distinction is
        load-bearing: `lost`, `unknown` and `ambiguous` are RESOLVED answers that
        legitimately carry no coordinate (`loc._record` fills `line`/`end` only on
        `current`, `moved` and `moved_file`), so the blanket "resolved implies a
        line" would be a false invariant that goes red on honest behaviour.

        The last assertion is the one that generalises: the hoisted pair must BE
        the record's own. `_summary` lifts `line`/`end` out of `resolution` for a
...
class TestTheMcpWrappersForwardTheFlag:
    ...
    def test_the_response_carries_the_summary_over_the_real_wire(self, tracker):
        ...
        # The key SET below is what survives serialisation; the VALUE has to be
        # asserted separately, because a summary carrying `"line": None` beside
        # `"resolved": True` satisfies every set-shaped assertion there is (CB-166).
...
class TestStalenessCarriesTheAnchorToo:
    ...
    def test_a_record_says_what_became_of_the_file_and_where_the_lines_went(self, tracker):
        ...
        # "where the lines went" is half this test's own name, and until CB-166
        # it asserted only the path. A `git mv` does not shift a line, so 4 is
        # both the stored and the resolved number and this cannot tell a
        # recomputation from an echo — that discrimination lives in
        # `TestTheNewLineNumberIsAnswered`. What it does hold is that this
        # surface, under its flag, carries a coordinate at all.
        assert record["anchor"]["line"] == 4
