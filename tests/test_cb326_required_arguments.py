...
class TestPlacementInTheChain:
    """Учёт вызовов обязан остаться таким, каким был. Оба теста зелены по обе
    стороны правки, и это их назначение.

    ЧЕСТНАЯ ОГОВОРКА, ДОБЫТАЯ МУТАЦИОННОЙ ПРОБОЙ И ИСПРАВЛЯЮЩАЯ ПРЕЖНЮЮ РЕДАКЦИЮ
    ЭТОГО ЖЕ ТЕКСТА. Здесь стояло, что перестановка слоя наружу учётного красит
    первую проверку, — и это было верно, пока слой ОТВЕЧАЛ САМ, не вызывая
    следующее звено: тогда снаружи учёта он отнимал у таблицы записи. После
    переустройства слой всегда вызывает следующее звено, поэтому учёт видит
    вызов с любой стороны от него, и мутант, двигающий установку наружу,
    оставляет проверку ниже ЗЕЛЁНОЙ. Замерено, а не предположено.

    Значит проверки этого класса стерегут не размещение, а САМ УЧЁТ: что отказ
    по-прежнему считается отказом инструмента, а отказ по незаявленному имени
    по-прежнему не считается вовсе. Размещение стережёт отдельная структурная
    проверка ниже — именно потому, что поведением его теперь не поймать.
    """

    def test_a_missing_field_refusal_is_still_counted_as_a_failure_of_that_tool(self, tracker):
        """Учёт обязан остаться прежним, и это не формальность: улика, которой
        обоснована карта CB-326, прочитана именно из таблицы `tool_calls`.
        Починка, незаметно перестающая считать эти отказы, обесценила бы
        собственное основание карты, ничего не изменив в ответе клиенту.

        Смотри оговорку в пояснении к классу: перестановкой слоя эту проверку
        больше не покрасить, потому что слой всегда вызывает следующее звено."""
        call_over_the_wire(
            _built(tracker), "add", {"category": "x", "file": "y", "description": "z"}
        )
