...
ПРЕДОСТЕРЕЖЕНИЕ ЧИТАТЕЛЮ, КУПЛЕННОЕ ДОРОГО, И ЕГО НАДО ПРОЧЕСТЬ ПРЕЖДЕ ВЫВОДОВ.
Словарь, который наблюдатель здесь печатает, — это то, что чужая библиотека УЖЕ
пропустила через свой сериализатор, а НЕ образец того, что допустимо вернуть ДО
него. Разница не теоретическая: по первому прогону этого пробника было решено,
что ответ можно собрать руками из тех же полей, — и такой ответ оказался негоден
в текущей редакции протокола, где у результата есть обязательное поле
`resultType`, которого в наблюдаемом словаре под старой редакцией нет. Клиент
получал не результат, а исключение. Поэтому пробник теперь снимает ОБЕ редакции
(`initialize()` и `discover()`), и разница между строками «слой видит» под ними
— и есть предмет предостережения. Правильный вывод из этого пробника только
один: подменять содержимое ЧУЖОГО конверта можно, строить свой — нельзя.
