...
DECLARED_EXCEPTIONS: dict[tuple[str, str], str] = {
    ...
    # unit (see the T-52 brief's Escalations section on the branch). The
    # DIR-1 holder's AST sweep for `@mcp.tool` decorators structurally could
    # not see these: sweep.py's MCP surface is generated by
    # `codebugs.surfacegen.emit_tools` from a data declaration in
    # `sweep_surface.py` (`dict(name="processed", type=bool, ...)`), not by a
    # literal `@mcp.tool()` decorator, so a textual/AST sweep for that
    # decorator syntax misses them while this ratchet's REGISTRY walk (which
    # goes through `provider.register_fn`, exactly what a real server does)
    # finds them regardless of how the tool got registered. Ownership
    # (DIR-1, DIR-2, or a third direction) and the file-budget consequence of
    # fixing them (they live in sweep_surface.py/surfacegen.py, neither of
    # which this unit's brief named) is the L2 holder's call, not this
    # unit's -- escalated rather than fixed unilaterally.
    ("codesweep_mark", "processed"): (
        "CB-151: population-measurement correction, ownership undetermined "
        "-- escalated to the DIR-1 holder (T-52 brief). sweep_surface.py's "
        "`type=bool` declaration, generated via codebugs.surfacegen; not "
        "reachable by an AST sweep for `@mcp.tool` decorators."
    ),
    ("codesweep_list_items", "include_archived"): (
        "CB-151: population-measurement correction, ownership undetermined "
        "-- escalated to the DIR-1 holder (T-52 brief). Same surfacegen "
        "declaration mechanism as codesweep_mark.processed."
    ),
    ("codesweep_list_items", "archived_only"): (
        "CB-151: population-measurement correction, ownership undetermined "
        "-- escalated to the DIR-1 holder (T-52 brief). Same surfacegen "
        "declaration mechanism as codesweep_mark.processed."
    ),
    ("codesweep_list", "include_archived"): (
        "CB-151: population-measurement correction, ownership undetermined "
        "-- escalated to the DIR-1 holder (T-52 brief). Same surfacegen "
        "declaration mechanism as codesweep_mark.processed."
    ),
