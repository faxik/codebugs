...
## Приёмка (3)←(4) — факт, 2026-08-20

Принято менеджером задачи по всем семи пунктам §13:
1. Дифф прочитан против замысла (merge-base 7a7e1bf): union на live+reopen, импорт не
   протекает, снятия нет, CB-29/`parse_tags`/`update(tags=)` не тронуты, зоны Т-8 не задеты.
2. Мутационная проба РУКАМИ: `if promote_tags:` → `if False and ...` в worktree —
   `test_reobserved_tag_is_found_by_query_tag` и `test_reopen_bump_unions_tags` КРАСНЫЕ
   (2 failed), восстановлено, дерево чистое.
3. Preflight-предпосылки перепроверены против приземлённого кода (символы сошлись;
   дрейф строк был отмечен заранее).
4. Слот эскалаций исполнителя: пуст, явно.
5. Этот раздел — неизменяемый факт приёмки в артефакте уровня.
6. Post-merge-механизм: в репо нет post-merge-gate — заменитель: worktree-finish
   (ruff + полная сюита на комбинированном дереве, merge --no-ff под flock) +
   CI-алярм main-invariants.yml. Результат — в мердж-SHA финиша.
7. Карт юнита НЕТ; population card CB-103 остаётся у DIR-2 (CLM-18) по плану посадки —
   терминальный статус не назначается этим юнитом.

TDD-свидетельство исполнителя: 8 тестов красные до реализации (коммит 6ca2b26,
`8 failed, 78 passed`), финально 1705 passed / ruff clean. Wire golden не тронут —
описания MCP-инструментов не менялись (правки только в приватном доменном слое).
