...
|---|---|---|---|---|---|
| 2026-08-17 | `codebugs` (pure/simple/confident) | CB-64 | **IN FLIGHT** — `codebugs claims --format table` crashes: `claims.py:781` passes `(columns, rows)` swapped into `format_table(rows, columns)` and builds list rows, not dicts. Codex pick-review concurred; batching refused (dry-run report: backlog resists clustering, no shared predicate among CB-64/67/68) | — | Codex re-ranked the tail: CB-67 (bench falsey-dispatch + CLI cross-arg) ahead of CB-68 (convention, arguably not a pure bug) |
