...
- **Т-31** — CB-140: проверка формы версии в гарде интерпретатора получила РАЗЛИЧАЮЩИЙ тест (`test_two_prefix_matching_non_versions_refuse_rather_than_agree`), `CLAUDE.md` приведён к тому, что держат тесты; `tools/_guards.sh` не изменён ни на байт — проверка была верна, не хватало пина. Мердж `c200aac`, принят DIR-1 2026-08-22 (7/7). Держатель проверил обе стороны лично (мутант, оставлявший 248/248 зелёными, краснит ровно новый тест); приёмщик поставил более острый мутант и показал, что старый тест на нём остаётся зелёным. Попутно закрыт остаток Т-28 (`tip-not-a-merge` → `tip-not-ours`). Приёмщик же нашёл, что причина дефекта была объяснена НЕВЕРНО в журнале и брифе (не порядок проверок, а переопределённость состояния) — исправлено.

- **Т-26** — CB-134: контракт закрытого stdout стал одним на 3.11–3.14 (`cli.run` отказывает до всякой работы кодом 141), bench-премиса строится по построению; найдена и закрыта ячейка, которой не было в карте — на 3.14 при fd 1, закрытом на exec, каждый верб отрабатывал МОЛЧА с rc 0, выбрасывая вывод. Мердж `8dea605`, принят DIR-1 2026-08-22 (7/7 §13, изолированный приёмщик). Сьют на main под документированной командой: 1949 passed (было «1 failed, 1942 passed»). Мутант «снять `sys.stdout = None`» ВЫЖИЛ — исполнитель переписал собственное утверждение вместо отписки. Побочно заведена CB-136.
- **Т-27** — CB-135: `.python-version` = 3.14.4 (полный patch) как единый источник для main, worktree и CI + гард `_guard_interpreter_matches_main` (exit 14) ДВАЖДЫ — предпроверкой и внутри замка; матрица `contracts` 3.11–3.14 в CI плюс сужение утверждения. Мердж `76b3d3b`, принят DIR-1 2026-08-22 с замечаниями. Свежий worktree и main дают ОДИН интерпретатор (проверено приёмщиком лично). Заявление исполнителя про убитый мутант ОПРОВЕРГНУТО приёмщиком и перепроверено держателем (248 passed под мутантом) → CB-140, юнит Т-31.
- **Т-29** — механика аллокатора CASCADE-IDS: `tools/cascade-mint.sh` вычисляет `max+1` под `flock` и коммитит только реестр одной операцией; тест на коллизию событийный и краснеет от обоих мутантов, включая замаскированный комментарием. Мердж `6f55e9c`, принят DIR-1 2026-08-22 с замечанием. Приёмщик ОПРОВЕРГ заявление автора о безопасности тестовых швов (шов писал по произвольному пути и урезал реестр до одной строки, попав в коммит) → CB-138, юнит Т-30. Скрипт применён в тот же день трижды по-настоящему (Т-30, Т-31 и минт четвёрки).
- **Т-30** — CB-138: способность шва писать по произвольному пути СНЯТА (форма «шва, которого нет, нельзя направить никуда»), handshake переехал на stderr и остался событийным. Мердж `00a0570`, принят DIR-1 2026-08-22 с замечаниями. Приёмщик прогнал три мутанта сам; тест доказывает НУЛЕВОЙ ЭФФЕКТ атаки, а не код возврата. Остатки → CB-141.
- **Т-28** — CB-121: пост-мердж-алярм (exit 15) на check-then-act внутрилоковой сверки SHA + сужение строки таблицы энфорсмента; тождество мерджи через `ORIG_HEAD`, а не счётом родителей (находка Codex раунда 2). Мердж `354cee7`, принят DIR-1 2026-08-22 с замечанием (устаревшее имя вердикта в комментарии). **Алярм смолчал на собственном приземлении** — живое подтверждение. Мутант M7 честно объявлен эквивалентным и проверен на интерпретаторе.
...
- **Т-25** (ex-Т-24, переминт после коллизии ID №3) — **CB-130**: оба отказа харнеса называют
  цену, которую платят ДРУГИЕ сессии. `pre-commit` на main печатает, что отказанные файлы остались
  в индексе, что хук намеренно не откатывает его, что до очистки любой `worktree-finish` в любом
  worktree клона отказывается с exit 11 — чужие включительно, — и даёт команду очистки;
  `_guard_main_clean` печатает, что грязь может быть чужой, с дискриминатором и подставленным
  путём. Мердж `e842952`, приземлён (3) 2026-08-22. Правка ТЕКСТА, поведение не тронуто. Пины
  ПРОГОНЯЮТ хук и гард (не грепают файл) и асертят сам механизм — `A  src/mod.py` в индексе и
  rc=11 на нём — ДО того, как смотреть на текст, так что сообщение не может пережить механизм,
  который описывает. Красный до правки 2 failed, после 2 passed; харнес-сьют 225, полный 1943,
  ruff чисто. **Юнит доказал сам себя на себе:** его finish трижды получил exit 13 «main moved»,
  и двигал main держатель — своими же заметками плана.
- **Т-23** (ex-Т-21, переминт после коллизии ID №2) — `_guard_enforcement_armed` требует третий
  хук (`commit-msg`) по монотонному условию «путь имеет историю на main» + абзац CI-limits п.4
  `CLAUDE.md` приведён к ИЗМЕРЕННОМУ `enforce_admins`. Мердж `3caced3`, принят DIR-1 2026-08-22.
  Строка леджера пропущена сессией 3 и дописана сессией 4 — отмечаю, а не подчищаю молча.
- **Т-20** — механизация К-3 (карты нет; процессная механика `tools/`): `commit-msg`-хук на main
  отказывает коммиту, стейджащему `.claude/plans/`-заметку, чьё имя не названо в сообщении —
  поимённость стала проверяемой. Мердж `b0e98de`, принят DIR-1 2026-08-21 условно, условие
  разрешено `97ecb39`. **Уровень (3) опроверг предписанную куратором форму ИЗМЕРЕНИЕМ**
  (`pre-commit` видит ПРЕДЫДУЩЕЕ сообщение — проверено мной лично) и перенёс на `commit-msg`;
  расхождение служит замыслу, принято, обе премисы запинены. Три ловушки сверх брифа, включая
  laundering через сепаратор в имени (воспроизведён, закрыт по построению). 9 мутантов, 7 убиты,
  2 эквивалента задекларированы. Условие приёмки: ложная фраза в `CLAUDE.md` про чистый
  cherry-pick — измерено, что секвенсер не достаёт ни до одного хука; снята и запинена тестом.
  Кросс-модельная половина неполная, названа вслух. **Долг: `_guard_enforcement_armed` ещё не
  требует новый хук — обязательство следующего юнита DIR-1.**
- **Т-18** — хвост CB-59 (карта уже была `fixed`): `CLAUDE.md` пункт 4 «CI job's own limits»
  приведён к ратифицированному скоупу (б) и к ИЗМЕРЕННОМУ состоянию защиты, мердж `27bdc5b`,
  исполнен и приземлён держателем DIR-1 лично 2026-08-21 (правка прозы, не кода; независимого
  приёмщика НЕ было — записано как слабость, а не выдано за приёмку). Сверх задания закрыто
  второе переобещание, которого хендофф не называл (абзац про доверенный upstream). **Находка:
  `enforce_admins` = `false`** — ратифицированные запреты force-push и удаления не связывают
  единственный пушащий аккаунт (он администратор); эскалация Э-9 владельцу, настройки его
  репозитория уровнем (2) не менялись.
- **Т-19** — CB-87 + CB-125 + CB-126 fixed (четыре незащищённых публичных входа обёрнуты в
  `db.txn`; DELETE доказывает себя `rowcount`-ом, UPDATE — `RETURNING`-ом), мердж `ff12974`,
  принят DIR-1 2026-08-21 (7/7 §13, /acceptance-check, приёмщик перепрогнал все четыре мутанта
  сам). Второй пакет полосы A. CB-125/CB-126 заведены мной при нарезке — популяция бралась
  ПРОГОНОМ `population.py`, а не из таблицы документа. Исполнитель добровольно сдал собственную
  методологическую ошибку (ложное «1 passed» из-за устаревшего `__pycache__`) → §13 п.12.
  Пятый сайт (`embeddings.batch_store_embeddings`) назван и не тронут: вред латентный.
- **BT-3** — матрица способностей (заказ владельца по В-5). v1 `646fc94` + вычислимый инструмент
  `.claude/plans/exposure-scripts/matrix.py`; гейт `adversarial-review-x2` **ПРОВАЛИЛ v1**
  (5 находок подтверждены обеими моделями независимо; Codex назвал недостачу 388 LOC до того,
  как её вычислили). v2 `a171d1b`: инструмент ключуется на ПРИМИТИВ регистрации вместо
  перечисления имён регистраторов, **прежняя рекомендация СНЯТА явно** — выигрыш был занижен в
  2.8 раза (пол 641, не 226), арка не «не оправдана», а «не ратифицируема пока», потому что
  цена M не измерена ни разу. **Второй проход провалил и v2** (критерий не мог ОТКЛОНЯТЬ; «647 LOC
  объявлений парсеров» — остаток вычитания, а не измерение; популяция (E) — шесть функций, а не
  четыре; `grouping.py` — отгруженная фича с записью в CHANGELOG, а не мусор). v3 `de4f8c4`:
  инструмент лишён последних двух перечислений по имени, **документ лишён рекомендации** —
  все восемь фатальных находок за два прохода лежали в интерпретации, ни одной в измерении.
  Наверх идут измерения (защитимо убираемо 587–1111 LOC из 3658; 530 LOC недостижимы) и два
  вопроса владельцу. Третий проход не запускается — названо вслух с основанием.
...
- **CB-103 закрыта** 2026-08-20 уровнем (2): контракт свежести полей дедуплицированной карты посажен целиком — 5 строк, 4 юнита (Т-8 `d3ff33f`, Т-9 `d491432`, Т-10 `0671881`, Т-11 `0466d44`), все приняты по §13. Открыто по воле владельца: суб-решение удаляемости тегов.
- **Т-12** — CB-61 ретро-fold категорий + CB-113(a): команда `categories-normalize` (dry-run по умолчанию, ре-деривация только auto:v1, round-trip перед ре-кеем, коллизии report-and-stop), мердж `8eff064`, принят DIR-2 2026-08-20 (7/7). CB-61 fixed; CB-113 намеренно оставлена открытой — fold к боевому трекеру не применён (решение владельца, операция необратима). Попутно заведена CB-122 (вакуозный TestLegacyWeirdRows).
- **Т-14** — ядро attention-блока BT-5 (сигнал `severity_escalated`, транспорт AddOutcome/BumpOutcome, живая матрица сигнал×ветка) + CB-118 + CB-119 fixed, мердж `303d7d5`, принят DIR-2 2026-08-20 (7/7, /acceptance-check со сквозной проверкой исполнением через MCP-стек).
- **Миграция данных (не юнит)** — ретро-fold боевого трекера применён 2026-08-20 по ратификации владельца: 17 строк (12 ре-деривация auto:v1, 5 только категория), 0 коллизий, 0 unverifiable; пост-fold dry-run даёт renames=0/unverifiable=0. **CB-113 закрыта** — путь (a) закрыт и в коде, и в данных. Бэкап: `<scratchpad>/codebugs-backup-PRE-APPLY-2026-08-20.csv`.
- **Т-13** — CB-108 + CB-120 fixed (три писателя `milestones/foundation.py` решают под замком
  и возвращают записанную строку; `item_ref`-only перечитывание растворено `RETURNING`, а не
  починено — `_get_item_row_by_ref` не тронут, это решение CB-33), мердж `53680a6`, принят
  DIR-1 2026-08-20 (7/7 §13, /acceptance-check). Первый пакет полосы A. Популяция BT-1 при
  этом сдвинулась 25 → 22, пересечение семей схлопнулось до нуля — измерено скриптом.
- **Т-15** — сигнал `category_divergence` блока attention + три правки CLAUDE.md (ре-негоциация индекса, закрытый retro-fold gap, контракт attention одним абзацем), мердж `6b3b3b6`, принят DIR-2 2026-08-20 (7/7). Менеджер упал с API-ошибкой после интеграции — работа сохранена артефактами. **Обе BT направления закрыты.**
- **Т-17** — CB-21 (parity-гейт над четырьмя источниками × две сущности + декларация трёх колонок как входов дериватного fingerprint) + CB-122 (вакуозный тест, вакуозность доказана контрольной пробой), мердж `6f3ceeb`, принят DIR-2 2026-08-21 (7/7). Поведение не менялось; CB-6 аннотирована грепаемым остатком.
- **Т-16** — CB-123: глагол `recent` («что закрылось с даты» одним вызовом MCP+CLI), оговорка «последнее касание ≠ момент закрытия» на трёх поверхностях и запиннена поведением, мердж `199329c`, принят DIR-2 2026-08-21 (7/7). Побочно заведена CB-124 (truthiness `--limit`). Открытый вопрос про таблицу истории статусов записан в карту с двусторонним доводом, а не решён молча.
- **Т-120** — CB-172 вариант (1) по решению куратора: шесть мёртвых маркеров `# noqa: S608` сняты, и абзац `CLAUDE.md` перестал выдавать маркеры за работающий механизм (правило `S608` в набор проверок не входит, секции правил в `pyproject.toml` нет вовсе). Мердж `3e52280`, DIR-1 2026-08-29, **ПРИНЯТ С ОГОВОРКОЙ двумя проходами по К-28**, апертуры разные намеренно. Первый (девять пунктов): обе оговорки адресованы уровню (2) и закрыты им в тот же час — перенацеливание CB-172 не было записано в самой карте, и цена долга «52 места в 17 файлах» смешивала две области (самосогласованно: 52 в 11 по `src/` либо 56 в 14 по `src/`+`tests/`; поправка приёмщика «52 в 14» починила не ту половину). Второй проход (правда абзаца как одного текста) нашёл то, чего первый не мог по построению: **фраза, приземлённая этим же юнитом, утверждает, что `RUF100` вскроет мёртвые маркеры других правил «в момент, когда что-либо включено», — опровергнуто построением, ноль `RUF100` при `select = ["S608"]`.** Ошибка того же рода, что юнит исправлял, в абзаце, который её разоблачает; ушла в **CB-264** вместе с двумя другими находками. **CB-172 НЕ закрыта — перенацелена на вариант (2)**, принятый долг, ратифицированное исключение пункта 7. Тир `sonnet` — совпал со стартовым.
- **Т-122** — CB-258: пробный каталог оракула CB-226 (`chmod 0300`) и вложенный прогон pytest ушли из живого рабочего дерева в одноразовую копию; обещание докстринга «нулевой след по построению», верное лишь для одного процесса, сужено до правды. Мердж `db53391` **с первой попытки**, DIR-1 2026-08-29, **ПРИНЯТ БЕЗ ОГОВОРОК**, CB-258 закрыта. Форма выбрана исполнителем ЗАМЕРОМ, как предписывал бриф: снятый держателем кандидат построен и подтверждён красным, цена копии измерена (0,016–0,08 с против вложенного прогона). Главное проверено приёмщиком лично — починка НЕ вакуумна: мутант, снимающий `onerror` из `conftest.py`, краснит тест, а мутант, возвращающий пробу в живое дерево, краснит на собственной страховке теста до `mkdir`. Набор 3261 зелёный. Сверх контракта: ряд CB-226 стал второй жертвой чтения живого дерева (учесть при CB-255), и число секунд в новом докстринге не воспроизвелось (0,41 с против обещанных 2,5) — в абзаце, который сам снимает счёт файлов с доводом «число в прозе гниёт». Тир `opus`; исполнитель сообщил, что для выбора формы он был избыточен, а окупился на дисциплине — из этого вырос пункт 20 шаблона брифа.
