...
### B14. Пример вывода `reqs-verify` не воспроизводится по форме

| | |
|---|---|
| **Утверждение (было)** | таблица без пустой строки перед заголовком и без строки-разделителя |
| **Команда** | импорт трёх требований, затем `reqs-verify` |
| **Вывод** | `Verified 3 requirements.` / пустая строка / `4 issue(s) found:` / **пустая строка** / заголовок / **строка-разделителя `------  ------  ------  ---…`** / строки |
| **Вердикт** | **ЛОЖНО по форме.** Блок заменён снятым прогоном, вместе с настоящими сообщениями трёх видов проверок |

### B15. Пример вывода `milestone-status` опускал два блока, которые команда всегда печатает

| | |
|---|---|
| **Утверждение (было)** | заголовок, строка `target:`, `Items:`, `Branch-only:`, `Blocked:` |
| **Команда** | наполнен `release/1.1` (три находки, одна на ветке, одна заблокирована, дата цели), затем `milestone-status release/1.1` |
| **Вывод** | дополнительно печатаются **строка описания этапа** под `target:` и **блок `By status:` / `By size:`** |
| **Вердикт** | **НЕПОЛНО.** Блок заменён снятым прогоном |

### B16. Сообщение гейта закрытия — одна строка, а не четыре, и из терминала недостижимо

| | |
|---|---|
| **Утверждение (было)** | блок с префиксом `ValueError:` в четыре строки с висячим отступом, поданный сразу после сессии `$ codebugs …` |
| **Команда** | `milestones.milestone_close(conn, id="release/1.1")`, вывод через `repr()` |
| **Вывод** | `'cannot close release/1.1: unfinished items (3): CB-1, CB-2, CB-3; branch-only items (1): CB-1@feat/CB-1; items with active blockers (1): CB-2  (use force=True with reason to override)'` — **одна неразрывная строка**, без переводов строки |
| **Вердикт** | **ЛОЖНО по форме, и вводило в заблуждение по достижимости.** У `milestone_close` нет верба CLI: показанное рядом с приглашением `$` внушало, что это можно набрать в терминале. Блок переписан как ошибка инструмента MCP, одной строкой |

Заодно проверено обещание про потоки: `milestone_close(id="stream/triage", force=True, reason="x")` →
`streams cannot be closed (milestone=stream/triage)`. **ВЕРНО.**

### B17. Раздел Development предлагал команду, переписывающую три четверти репозитория

| | |
|---|---|
| **Утверждение (было)** | `# Format` / `uv run ruff format src/ tests/` |
| **Команда** | `ruff format --check src/ tests/` (неразрушающая форма) |
| **Вывод** | `75 files would be reformatted, 25 files already formatted` |
| **Вердикт** | **ВВОДИТ В ЗАБЛУЖДЕНИЕ.** `ruff format` в этом дереве намеренно не гейт (так сказано и в `CLAUDE.md`), а README подавал его как обычную команду разработчика. Убран, причина названа. Заодно к `pytest` и `ruff` добавлен `--extra dev`, без которого свежий клон падает с `No module named pytest` |

### B18. Флаги примеров — проверены поимённо, ни одного выдуманного

| | |
|---|---|
| **Команда** | все командные примеры sweep, требований и этапов исполнены дословно |
| **Вывод** | `sweep-create --batch-size` код 0; `sweep-add --tags` код 0; `sweep-create --lifecycle/--terminal-states` код 0 (печатает цепочку жизненного цикла); `sweep-mark --state` код 0; `sweep-archive-items --state --older-than 30d` код 0; `milestone-audit --milestone` код 0; `update --append-note` код 0; `reqs-query --status Implemented --priority Must` код 0 — словарь разрешается без учёта регистра; `reqs-update --notes` код 0 |
| **Вердикт** | **ВСЕ СУЩЕСТВУЮТ И РАБОТАЮТ.** Ни одного выдуманного флага не найдено |
