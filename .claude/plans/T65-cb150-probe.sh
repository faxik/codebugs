...
echo "=========================================================="
echo "# CB-150/T-70 NOTE: on LANDED main, expect MORE than 12 refusals here,"
echo "# with the extra ones starting at the transition INTO T-65's own"
echo "# record commit (and continuing for one or more transitions after it,"
echo "# growing as main advances). That is NOT a live false refusal: this"
echo "# replay LINEARIZES a branching history with --follow --reverse, and"
echo "# T-65's branch was cut before a later mint (Т-69) landed on main by a"
echo "# different path, so linear replay reads the branch's own registry"
echo "# blob as having DELETED that mint. See CASCADE-IDS.md's own header"
echo "# (the T-65/T-70 record) for the full explanation — the 12/<=23"
echo "# figure names refusals on the FIRST-PARENT line up to the gate's own"
echo "# introduction; anything reported past that index on a fresh run is"
echo "# this linearization artifact, not a new false refusal."
d="$(mk_probe partA)"
cd "${d}"
