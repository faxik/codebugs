...
The `...` are elision marks rather than output: this is one entry lifted out of a larger JSON document, and the anchor object carries one further key below `survived` — `resolved_against`, the root, commit and on-disk file the answer was computed against. Nothing is ever omitted because it was absent. Every key is present on every answer, so `"reason": null` here means *there is nothing to explain, because there is an answer* — never *this field is missing*.
